var heightFilterFactory = [
    "uomService",
    "conversionService", 
    function (uomSvc, conversionSvc) {
        return function (input, convert) {
        
        var heightInches = Number(input), heightCentimeters, ft, result = '';
        ...
        if (uomSvc.usMeasure) {
                ...
                if (convert !== undefined && !!convert === convert && !convert) {
                    ...
                    heightCentimeters = Math.round(
                        conversionSvc.inchesToCentimeters(heightInches), 1);
                }
